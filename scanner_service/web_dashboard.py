    ...
    def _setup_socket_events(self):
        ...
        def handle_start_scan(data):
            ...
            try:
                ...
                if scan_type == 'quick':
                    # For quick scan, scan the full Mac (root directory)
                    scan_path = '/'
                elif scan_type == 'deep':
                    # For deep scan, use provided path or default to user home
                    scan_path = path if path else '/Users'
                else:
                    scan_path = path if path else '/'

                logger.info(f"Starting {scan_type} scan of {scan_path}")
                emit('scan_started', {
                    'path': scan_path,
                    'type': scan_type,
                ...
                def run_scan():
                    try:
                        # Call your backend scan logic here
                        # This should scan scan_path and emit results as they come in
                        results = self.db.run_full_scan(scan_path)
